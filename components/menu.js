...
const NormalButton = ({ href, name }) => {
  ...
  return (
    <NextLink href={href === router.asPath ? '#' : href}>
    ...
    <Menu>
      ...
      <MenuList fontSize={'1rem'}>
        {sublinks.map((item, index) => (
          <NextLink key={index} href={prefix + item.href === router.asPath ? '#' : prefix + item.href}>
            <MenuItem color="black">
              <Link style={{ textDecoration: 'none' }}>{item.name}</Link>
            </MenuItem>
          </NextLink>
        ))}
      </MenuList>
    ...
    <Box
      ...
      <HStack justify={'space-evenly'}>
        {menuLinks.map((item, index) =>
          item.sublinks ? (
            <ListButton key={index} name={item.name} prefix={item.href} sublinks={item.sublinks} />
          ) : (
            <NormalButton key={index} name={item.name} href={item.href} />
          )
        )}
      </HStack>
    </Box>
